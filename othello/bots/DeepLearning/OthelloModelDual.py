...
class OthelloModel():
    def __init__(self, input_shape=(8,8)):
        self.model_name='modelDual_'+( 'x'.join(list(map(str, input_shape))) )+'.h5'
        ...
        self.v=Dense(1, kernel_regularizer=l2(0.0005))(gap1)
        self.v=Activation('tanh',name='v')(self.v)
        self.model = Model(inputs=self.input_boards, outputs=[self.pi,self.v])
        self.model.compile(loss=['categorical_crossentropy', 'mse'], optimizer=Adam(0.002))
        
    def predict(self, board):
        p=self.model.predict(np.array([board]).astype('float32'))
        return p[0],p[1]
    
    def fit(self, data, batch_size, epochs):
        input_boards, target_policys,values = list(zip(*data))
        input_boards = np.array(input_boards)
        target_policys = np.array(target_policys)
        values=np.array(values)
        #print(values)
        #print(target_policys)
        self.model.fit(x = input_boards, y = [target_policys,values], batch_size = batch_size, epochs = epochs)
    ...
    def load_weights(self):
        print('othello/bots/DeepLearning/models/'+self.model_name)
